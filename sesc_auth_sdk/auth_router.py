...
from sesc_auth_sdk.schemas.token import TokenResponse, AuthentikTokenResponse, TokenRequest, LogoutResponse
...
def _clear_initial_oauth_code_flow_cookies(response: Response):
    ...
    response.delete_cookie(key='oauth_nonce', path='/auth')
...
@router.post("/token", response_model_exclude_unset=True)
async def token(body: TokenRequest, request: Request, response: Response) -> TokenResponse:
    if body.grant_type == 'authorization_code':
        ...
        if not cookie_code_verifier or not cookie_state or not body.code or not body.state or body.state != cookie_state or not cookie_nonce:
