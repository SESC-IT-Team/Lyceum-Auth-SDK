...
## 1. Интеграция в проект
  - ### Установка пакета в проект  
    ...
    ```
    Теперь пакет можно добавить в uv как обычный пакет с PyPI или другого источника, просто написав команду в консоли:  
    ```bash
  ...
  - ### Конфигурация .env
    ...
    _Примечание: в .env файле могут быть и другие переменные нужные в Вашем проекте_
## 2. Использование
  Пусть у вас есть эндпоинт / и вы хотите, чтобы он был доступен только авторизованным пользователям, тогда делаете так:
  ```python
...
def index(payload: JwtPayload = Depends(LyceumAuth(allowed_roles=[Role.admin]))):
    ...
  ```
Такая реализация положит в payload только информацию, которая хранится в JWT.  
Если вы хотите при это м получить полную информация про пользователя, то делайте так:
  ```python
from fastapi import FastAPI, Depends
from sesc_auth_sdk.dependencies import LyceumAuth
...
def index(user: UserSchema = Depends(LyceumAuth(allowed_roles=[Role.admin]).return_user)):
    ...
  ```
  Разница в том, что в первом случае токен валидируется локально (в большинстве случаев без запросов к бэкенду авторизации, запросы к бэку придётся делать только для обновления ключей), в то же время, когда возвращается пользователь, точно нужно будет запросить у бэка авторизации данные этого пользователя
