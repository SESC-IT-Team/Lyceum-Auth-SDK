...
def index(payload: JwtPayload = Depends(LyceumAuth())):
  ...
  Если вы хотите допустить до эндпоинта только определенные роли, то делаете так:
  ```python
from fastapi import FastAPI, Depends
from sesc_auth_sdk.dependencies import LyceumAuth
from sesc_auth_sdk.schemas.jwt import JwtPayload
...
Если вы хотите при этом получить полную информация о пользователе, то делайте так:
  ```python
from fastapi import FastAPI, Depends
from sesc_auth_sdk.dependencies import LyceumAuth
